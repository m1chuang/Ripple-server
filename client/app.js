...
function ExploreUI (apiClient, data){
  this.data = data;
  this.api=apiClient;
  this.data.explore_list.subscribe = function(index){
    console.log('substribing to explore item: '+index);
    this.api.post('/moment/action', {action_token:this.items[index].action_token.subscribe},function(response){
      console.log(response);
    })
...
function SubscribeUI(apiClient, data){
  ...
  this.nextPage = function(data){
    ...
    this.data.friend_list.push(2);
    return this
  }
  this.refresh = function(data){
    console.log('api call to subscriber refresh');
    this.data.explore_list.push(2);
    return tihs
  }
};
...
App.prototype.subscribe = function(index){
  this.data.explore_list.subscribe(index,function(response){
    console.log(response);
  })
};
