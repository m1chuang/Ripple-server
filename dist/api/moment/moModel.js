...
var createExplore = function (nearby_moments, next) {
    if (!nearby_moments) {
        ...
        async.mapSeries(nearby_moments, generate_like, function onExploreGenerate(err, explore_list) {
...
MomentSchema.statics.isExpired = function (token, next) {
    this.model("Moment").find({
        actor_id: token.actor_id }).limit(1).exec(function (err, mo) {
        if (err || mo.length === 0) {
            next(true);
...
MomentSchema.methods.getNearWithRelation = function (params, next) {
    ...
    mongoose.model("Moment").find({
        ...
        connection: {
            $elemMatch: {
                target_mid: params.my_mid }
        } }).skip(params.offset).limit(params.limit).exec(function (err, nearby_moments) {
        if (err) throw err;
