...
exports.initMoment = function (params, response) {
    ...
    var newActor = new ACTOR({
        ...
        health: "established" });
    AUTH.newAuthToken({ device_id: device.device_id, actor_id: actor_id, lat: params.lat, lon: params.lon }, false, function (newToken) {
        response(202, "", newToken);
    });
    MOMENT.getExplore(params, function saveExploreList(err, explore_list) {
        LOG.info(CHALK.red("In MOMENT.init end"));
        newActor.explore = explore_list || [];
        newActor.save(function (err) {
            LOG.error("eruuuuur");LOG.error(err);
        });
    });
};
...
var actionMenu = {
    ...
    */
            ...
            if (err || !actor[0]) {
                ...
                if (actor[0].relation[0] && actor[0].relation[0].actor_id === target_aid) {
                    if (!actor[0].connection[0] && actor[0].relation[0].status === 0) {
                        PUBNUB.createConversation(actor.pubnub_key, actor[0].relation[0].pubnub_key, function addConnections(channel_id) {
                            ACTOR.saveRemoteConnection({ own_device_id: params.auth_token.device_id, owner_aid: params.auth_token.actor_id, target_aid: target_aid, type: "like", channel_id: channel_id }, function (err, device_id) {
                                ...
                                PUBNUB.notifyRemote({
                                    ...
                                    server_channel_id: device_id }, function () {});
                                next(202, {
                                    channel_id: channel_id }, "liked by target");
                            });
                        });
    ...
    subscribe: function (params, next) {
        ...
        DEVICE.addSubscriber(target_did, params.auth_token.device_id, params.body.nickname || "", function (err) {
            next(200, {});
        });
    }
};
